...
TARGET_SCENE_COUNT = (5, 8) # inclusive range for 70s video
VALID_FRAME_RANGE = (90, 360)  # 3s–12s per scene @ 30fps
MAX_HOOK_WORDS = 15         # hook narration word count ceiling
IDEAL_HOOK_WORDS = 10       # word count for full hook score
...
REQUIRED_SCENE_TYPES = {"HookScene", "CTAScene"}
VALID_SCENE_TYPES = {
    "HookScene", "ExplanationScene", "TerminalScene",
    "MapPingScene", "SplitViewScene", "CTAScene",
}
ACCENT_COLOR = "#00ff41"
...
def score_script(script: dict) -> dict:
    ...
    hook_narration = hook.get("narration", "")
    hook_words = hook_narration.split()
    hook_word_count = len(hook_words)

    hook_score = 5
    if hook_type != "HookScene":
        hook_score -= 2
        issues.append(f"hook: first scene type is '{hook_type}', expected 'HookScene'")
    ...
    filler_hits = [w for w in FILLER_WORDS if w in hook_narration.lower()]
    if filler_hits:
        hook_score -= 1
    ...
    for s in scenes:
        ...
        # Headline max 5 words (from skill spec)
        if headline and len(headline.split()) > 5:
            visual_score -= 1
            issues.append(
                f"scene '{sid}': headline too long ({len(headline.split())} words, max 5)"
            )

    # Required scene types present
    actual_types = {s.get("type") for s in scenes}
    missing = REQUIRED_SCENE_TYPES - actual_types
    if missing:
        visual_score -= 1
        issues.append(f"missing required scene types: {missing}")
    ...
    arc_score = 5
    n = len(scenes)
    lo_arc, hi_arc = TARGET_SCENE_COUNT

    if not (lo_arc <= n <= hi_arc):
        arc_score -= 2
        issues.append(f"arc: {n} scenes (target {lo_arc}–{hi_arc} for 70s video)")

    last = scenes[-1]
    if last.get("type") != "CTAScene":
        arc_score -= 1
        issues.append(f"arc: last scene is '{last.get('type')}', expected 'CTAScene'")

    # Narration must be present on every scene
    missing_narration = [s.get("id", "?") for s in scenes if not s.get("narration", "").strip()]
